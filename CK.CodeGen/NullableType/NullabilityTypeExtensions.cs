...
namespace CK.CodeGen
{
    ...
    public static class NullabilityTypeExtensions
    {
        ...
        /// <summary>
        /// Creates a <see cref="NullabilityTypeTree"/> for a parameter's type.
        /// <param name="this">This parameter.</param>
        /// <returns>The nullable tree for the parameter' type.</returns>
        [DebuggerStepThrough]
        public static NullableTypeInfo GetNullableTypeInfo( this ParameterInfo @this )
        {
            var info = GetNullabilityInfo( @this );
            return GetNullableTypeInfo( @this.ParameterType, info );
        }
        ...
        /// <summary>
        /// Creates a <see cref="NullabilityTypeTree"/> for a property's type.
        /// <param name="this">This property.</param>
        /// <returns>The nullable tree for the property's type.</returns>
        [DebuggerStepThrough]
        public static NullableTypeInfo GetNullableTypeInfo( this PropertyInfo @this )
        {
            var info = GetNullabilityInfo( @this );
            return GetNullableTypeInfo( @this.PropertyType, info );
        }
        ...
        /// <summary>
        /// Creates a <see cref="NullabilityTypeTree"/> for a field's type.
        /// <param name="this">This field.</param>
        /// <returns>The nullable tree for the fields's type.</returns>
        [DebuggerStepThrough]
        public static NullableTypeInfo GetNullableTypeInfo( this FieldInfo @this )
        {
            var info = GetNullabilityInfo( @this );
            return GetNullableTypeInfo( @this.FieldType, info );
        }
        ...
        /// <summary>
        /// Creates a <see cref="NullableTypeInfo"/> for this type based on a root <see cref="NullabilityTypeInfo"/> that must
        /// have been computed for this type otherwise behavior is undefined. 
        /// </summary>
        /// <param name="this">This type.</param>
        /// <param name="info">The nullability info.</param>
        /// <returns>The detailed, recursive, <see cref="NullableTypeTree"/>.</returns>
        [DebuggerStepThrough]
        public static NullableTypeInfo GetNullableTypeInfo( this Type @this, NullabilityTypeInfo info )
        {
            var annotations = info.GenerateAnnotations().GetEnumerator();
            ...
            if( decl != null )
            {
                ...
                while( decl != null );
                var trees = new NullableTypeTree[pathTypes.Count];
                for( int i = 0; i < trees.Length; ++i )
                {
                    var k = i == trees.Length - 1 ? info.Kind : default;
                    trees[i] = GetNullableTypeInfoTree( pathTypes.Pop(), annotations, k, i == 0 );
                }
                return new NullableTypeInfo( trees );
            }
            return new NullableTypeInfo( new[] { GetNullableTypeInfoTree( @this, annotations, info.Kind, true ) } );
        }

        static NullableTypeTree GetNullableTypeInfoTree( Type t, IEnumerator<byte> annotations, NullabilityTypeKind known, bool isFirst )
        {
            NullableTypeTree[] sub = Array.Empty<NullableTypeTree>();
            if( known == NullabilityTypeKind.Unknown )
            {
                known = t.GetNullabilityKind();
            }
            if( known.IsNullableValueType() )
            ...
            if( !isFirst && !known.IsNonGenericValueType() )
            {
                // Consume our annotation.
                if( !annotations.MoveNext() )
                {
            ...
            {
                Debug.Assert( known.IsReferenceType() );
                sub = new[] { GetNullableTypeInfoTree( t.GetElementType()!, annotations, default, false ) };
            }
            else if( t.IsGenericType )
            {
                Debug.Assert( (known & NullabilityTypeKind.IsGenericType) != 0, "This has been already computed." );
                ...
                foreach( var g in genArgs )
                {
                    sub[idx] = GetNullableTypeInfoTree( g, annotations, default, false );
                    ++idx;
                }
            }
            return new NullableTypeTree( t, known, sub );
